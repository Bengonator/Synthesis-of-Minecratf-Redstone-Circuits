...
(begin
  ...
  ; (add_str_block_goal name operator str (col[s] row[s] [is_initial #t])
  (define (add_str_block_goal name operator str col row [is_initial #t])
  ...
  (define (solve_world)
    ...
    (define (print_layers binding n_prints)
      ...
      (when (> n_prints 0)
        ...
        (define max_prints (length PRINT_LAYERS))
        (define last_print (last (indexes-of PRINT_LAYERS #t)))
...
(define SPECIFIED_COSTS '(99  ; error
                          0   ; air
                          1   ; solid block
                          9   ; redstone block
                          2   ; redstone dust
...
; (add_str_block_goal name operator str col[s] row[s] [is_initial])
